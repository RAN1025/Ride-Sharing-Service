...
def display(request, id):
    ...
    for i in range(0, len(confirmedrides)):
        ...
        vehicle_plate = get_object_or_404(UserProfile, user=curt_driver).plate
        ...
        cfm_info_rider.append(
            {'destination': curt_destination, 'arrivaldate': curt_arrivaldate, 'driver_name': driver_name,
             'vehicle_info': vehicle_info, 'vehicle_plate': vehicle_plate, 'sharers': temp_sharers,
             'id': confirmedrides[i].id,
        ...
        if id in one.sharer_id:
            ...
            confirmedshares.append(
                {'rider_name': rider_name, 'destination': one.destination, 'arrivaldate': one.arrivaldate,
                 'driver_name': driver_name, 'vehicle_info': vehicle_info, 'vehicle_plate': vehicle_plate})
...
def newride(request, id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            ride = Ride(destination=destination, arrivaldate=arrivaldate, passenger=passenger, sharable=sharable,
                        vehicle=vehicle, special=special, rider_id=rider_id)
            ride.save()
...
def editinfo(request, id):
    ...
    if request.method == 'POST':
        # check whether existing drive
        exist = Ride.objects.filter(status='confirmed', driver_id=id)
        if len(list(exist)) > 0:
            return HttpResponseRedirect(reverse('users:profile', args=[user.id]))
        form = DriverForm(request.POST)
